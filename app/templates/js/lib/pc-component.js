...
$.fn.tab({
	...
	move_time : 500  从一个切换到另外一个的时间
	phone_tipbar : true    在手机上显示iscroll提供的滚动条，默认不显示
});
*
*/
$.fn.tab = function(param){
	var timeid,intervalid,content,plistli,nowli,nowdiv,preli,prediv,tab, plist, listafter, pcontent, action, len, nowindex,time,type,leftarr,rightarr,myScroll,flag_animate,phone_tipbar,access_ctrl,t1,li_width,sec=1;
	ptab = '.' + param.tab;
	plist = '.' + param.plist;
	...
	li_width = parseInt(content.css('width'));  //获取每个li的宽度，用以给move1和move2作为移动距离
  	plistli = $(plist).children('li');
	tab = $(ptab);//
	len = plistli.length;			//菜单条li总个数
	if(!('ontouchstart' in document.documentElement)) {   //是PC的话
		if(type != 'move1' && type != 'move2')content.hide();  //move1和move2都是不需要隐藏的
	}
	t1 = new Date().getTime();   //防止用户频繁点击
	$(plist).one("focusin", function(){   //这个应该是给tabs的键盘交互事件
		$(document).keydown(function(e){
	    	var varkey = (e.keyCode) || (e.which) || (e.charCode); //兼容各个浏览器
	    	var target = $('.' + listafter);
	    	switch (varkey){
	    		case 39:  //向右键
		    		switchTab(nowindex, nowindex + 1);
					$('.' + listafter).trigger('focus'); //只有focus才能读出选项卡里面的菜单，这个值是需要动态获取的
					break;
				case 37:  //向左键
		    		switchTab(nowindex, nowindex - 1);
					$('.' + listafter).trigger('focus');
					break;
				case 27:  //esc键关闭，退出tabs模块
					$('#tabs-esc').unbind("focus").focus();
					$(document).unbind("keydown").unbind("keyup");  //在移除tab选项卡之后要清除绑定的键盘事件
					break;
				case 18:  //alt键，组合键
					access_ctrl = true;
					break;
				case 33:  //page up，这里采用alt+pageup到上一个选项卡
					if(access_ctrl){
		        		target.prev().focus();
		        	}
					break;
				case 34: //page down，这里采用alt+pagedown到下一个选项卡
					if(access_ctrl){
		        		target.next().focus();
		        	}
					break;
	    ...
	    $(document).keyup(function(e){   //当alt没有按下的时候，组合键失效
	    	var varkey = (e.keyCode) || (e.which) || (e.charCode); //兼容各个浏览器
			if(varkey == 18){
				access_ctrl = false;
			}
	...
	$('#tabs-esc').bind('focus', function(event) {  //退出是模块最后一个链接，用户是不能通过tab退出模块的，必需要使用esc键
		$(pcontent + " li:visible").focus();
	});
	//初始化操作
	content.eq(--nowindex).show();
	if(type == 'move2' && len != content.length){  //是需要将ul左移动，值为  -（每个li的宽度*个数-左边箭头的宽度）
		content.parent().css('marginLeft', -li_width*len + parseInt($('.' + param.leftarr).css('width')));
	}
	plistli.eq(nowindex).addClass(listafter);
	plistli.each(function(index){  //给菜单绑定click，hover事件
		if(action=='click'){
			$(this).click(function(){
        ...
        $(this).focus(function(){   //给使用键盘tab键可以实现效果
            switchTab(nowindex, index);
            $(this).keydown(function(e){   //当alt没有按下的时候，组合键失效
            	var url = $(this).attr('data-url');
            	if(!url)return;
		    	var varkey = (e.keyCode) || (e.which) || (e.charCode); //兼容各个浏览器
				if(varkey == 13){  //当输入回车的时候在新窗口打开链接
					window.open(url);
				}
			});
        });
	...
	if(param.leftarr){
		...
		leftarr.click(function(){
			if(type == 'move2'){
				switchTab(nowindex, nowindex - 1, 'left');  //调用了后面封装的函数
			} else {
				switchTab(nowindex, nowindex - 1);
			}
        });
	...
	if(param.rightarr){
		...
		rightarr.click(function(){
			if(type == 'move2'){
				switchTab(nowindex, nowindex + 1, 'right'); //move2才需要最后这个参数，但是如果不判断会对move1产生影响
			} else {
	...
	function switchTab(pre, now, animate_phone){
		var t2 = new Date().getTime();   //本次点击的时间
		if(t2 - t1 < 600)return;        //如果这个间距小于一定的值就不执行下面的函数
		...
		switch (type){
			...
			case 'move1':
						...
						if('ontouchstart' in document.documentElement) {
							...
							content.parent().parent().animate({scrollLeft: parseInt(li_width)*now}, {queue:false, duration:800});
						}
						break;
			case 'move2':
						var _tem = $(pcontent).children().children(); //因为dom节点在变化，所以每次都要重新获取
						var first = _tem.first();  //第一个元素
				        if(animate_phone == 'left'){
				            var last = _tem.last();  //第一个元素
				            first.animate({marginLeft: li_width}, 500, function(){
				                first.removeAttr("style");
				                last.insertBefore(first);
				            });
				        } else if(animate_phone == 'right'){
				            var change = first.clone();   //克隆第一份元素，最后是要放到最后面的，因为原始的是含有marginLeft值了
				            first.animate({marginLeft: -li_width}, 500, function(){
				                $('.com-tab-content ul').append(change);
				                first.remove();  //把第一份的删除
				            });
				        } else {
			                var sec = (now - pre +len)%len;  //需要向右移动的次数
			                var li_list = $(pcontent).children().children();  //缓存li结构并形成一个闭包给后面使用
		                	for(var i=0;i<sec;i++){
		                		var li_now = li_list.eq(i);
		                		li_now.animate({marginLeft: -li_width}, 500, (function(i){ //采用了闭包
		                			return function(){
			                			var tt = li_list.eq(i);
		                    			var change = tt.clone();
...
$.fn.input = function(param){
	var target,action,tiparea,label,input_focus,input_put, input_type;
	...
	target.focus(function(){
		...
		target.parent().addClass(input_focus);
	});
	target.blur(function(){//离开输入状态，文本框中没有值，则显示提示值并让字体变成提示的样式
		if(!target.val()){
			target.parent().removeClass(input_focus);
			label.show();
		}
	});
	if(action == 'hover'){
...
$.fn.popwin = function(param){
	...
	var mainfunc = function(){
        ...
        $(document).bind('focusin', function(event) {
        	if(event.target.nodeName != 'DIV') {   //防止冒泡，因为弹窗是可以被focus的，所以要去除
...
$.fn.dial({
    ...
    vertical : 3,   //竖排的个数
    horizontal : 3,  //横排的个数
    times_url: 'http://localhost:8125/',  //获取次数的的url
    times_func: function(data) {
    	data = JSON.parse(data);
        if(data.flag){  //这个是验证数据是否正确的，如果正确则为true
        	$('.com-btn-reward span').html(data.num); //返回来的抽奖次数，可修改
        	if(data.num < 0){
        		alert('你没有抽奖机会了') //可修改
        		return false;
        	}
    ...
    result_url: 'http://localhost:8124/',   //用户抽奖请求的url
    result_func: function(data) {
    	if(!data.flag){   //如果是数据的错误，提示退出
    		alert(data.errmsg);
    		return;
    	}
    	msg = data.string;
    	var nextid, position;
        position = ["t1023","t1025","t1021","t1024","t1026","t1022","t1027","t1028"];  //根据后台的奖品ID来排序，数组的顺序就是转动的顺序
        for(var i=0;i<position.length;i++){
        	if(position[i] == data.id){
        		nextid = i+1;       //服务器返回应该去的位置
        	}
        }
        var after_msg = {'msg': data.string, 'nextid': nextid};  //将从后台获取的数据返回给js
        return after_msg;
    },
    over_sult: function(){
    	alert(msg);  //回调函数，可修改，就是转盘转完后执行的函数
    }
});
 *
 */
$.fn.dial = function(param){
    var nowposition, nextid, tid, count, circle, obj, btn, left, top, width, height, ttop, tleft, mtop, mleft, num;
    obj = '.' + param.obj;
    btn = '.' + param.btn;
    left = param.left;
    top = param.top;
    ...
    function round(){  //控制变向的转动
        ...
        if(--count < 1){
            clearInterval(tid);  //停止转动
            param.over_sult();   //回调函数，一次抽奖结束的动作
        }
    }

    function animate(){   //控制转动的核心代码
    	if(nextid){  //如果不是第一次转（即是当前的状态不是第一个），就把上一次的位置做为第一次
            nowposition = nextid;
        } else {
            nowposition = 1; 
        }
        nextid = after_msg.nextid;
    ...
    $.ajax({   //页面刚开始或刷新的时候需要去服务器拉用户还有的机会次数
        url: param.times_url,
        type: 'POST', 
        cache: false,
        success: param.times_func,
    ...
    btn.click(function(){
        if(count>0)return;  //屏蔽了频繁的点击，不然多次点击发送对后台也有影响
 		$.ajax({
	        url: param.times_url,
	        type: 'POST',
	        cache: false,
	        success: function(data){
	        	if(param.times_func(data)) {  //如果返回为true才执行转盘，如果没有机会了就不会执行下面的异步拉取代码
	        		$.ajax({
				        url: param.result_url,
				        type: 'POST', 
				        cache: false,
				        success: function(data) {
				        	data = JSON.parse(data);
				        	after_msg = param.result_func(data);
					        animate();
				        },
				        error: function(jqXHR, textStatus, errorThrown) {
				            alert('error ' + textStatus + " " + errorThrown);
...
/*
 *#9.position：fixed；只是修复IE6的fixed bug，支持top和bottom定位，IE6的实现方法有个技巧，具体参考我的博客。
 还可以实现返回顶部的功能，到指定的位置出现消失gototop
 *调用函数方法
$.fn.fixed({
       target : '.a1',
       gototop : 300 
...
$.fn.fixed({
       target : '.a1',       目标元素
       gototop : 300         当scrolltop等于300的时候就显示返回顶部
 });
 *
 */
$.fn.fixed = function(param){
    var fixed_target, fixed_type, st, d_height, w_height, is_ie6, scroll_top;
    fixed_target = $(param.target);
    
	st = $(window).scrollTop();
	d_height = $(document).height();
	w_height = $(window).height();
	fixed_target.value = parseInt(fixed_target.css('top'));  //有一个被覆盖的问题，如果一个页面有多个的话
	is_ie6 = $.browser.msie && $.browser.version<7;
	scroll_top = param.scroll_top;
	flag = true;
	
	if(!isNaN(fixed_target.value)){      //就是判断在css里面写的是是bottom还是top，并将值取出来
		fixed_type = "top";
	} else {
		fixed_type = "bottom";
		fixed_target.value = parseInt(fixed_target.css('bottom'));
	}
	
	function fix_ie6_bottom(){   //将元素基于document来定位
		if(is_ie6){
			if(fixed_type == "bottom"){
				st = $(window).scrollTop();
				fixed_target.css('bottom', fixed_target.value + d_height - w_height - st);
			}
		}
	}
	fix_ie6_bottom();
	$(window).scroll(function(){
        if(is_ie6){
        	if(fixed_type == "bottom"){
        		...
        		st = $(window).scrollTop();
        		fixed_target.css('top', st + fixed_target.value);
        	}
		}
		if(scroll_top){
			st = $(window).scrollTop();
			if(st>scroll_top){       //这里没有做处理，在复杂的情况下是可以优化的
				fixed_target.fadeIn(1000);
			} else {
				fixed_target.fadeOut(1000);
			}
		}
    });
    $(window).resize(function(){  //resize只需要对bottom重排
    	w_height = $(window).height();  //这个时候需要重新计算这个值
    	fix_ie6_bottom();
    });
};
 ...
 */
 $.fn.scAnimate = function(param){
    ...
    if(isNaN(st_position)){
    	var st_obj = $('' + st_position[0]);
    	st_position = st_obj.offset().top + parseInt(st_position[1]);
    }
...
/*
 *#12.绝对定位和固定定位切换，这个组件目前还很局限，因为主要的需求就是这样，后期可以扩展
 *调用函数方法
$.fn.fixed_change({
       target : '.a1',
       after_class : 'change1',
...
$.fn.fixed_change({
       target : '.a1',       目标元素
       after_class : 'change1',     改变之后的类，主要是给除IE6的其他浏览器使用的
       work_line : 300              浏览器滚动的高度
});
 *
 */
$.fn.fixed_change = function(param){
    var fixc_target, fixc_work_line, fixc_bottom, fixc_top, fixc_before;
    fixc_target = $(param.target);
    
	fixc_work_line = param.work_line;
	fixc_top =false;
	fixc_bottom=true;
	fixc_change = param.after_class;
	is_ie6 = $.browser.msie && $.browser.version<7;
	d_height = $(document).height();
	w_height = $(window).height();
	fixc_before = parseInt(fixc_target.css("top"));  //开始的高度值


	$(window).scroll(function(){
		st = $(window).scrollTop();
		if(is_ie6){    //IE6的处理
			if(st < fixc_work_line){  //对采用fixed和绝对定位的分界点
	            	fixc_target.css("top", fixc_before);
	        } else {
	            	fixc_target.css("top", fixc_before - fixc_work_line + st);
	        }   
	        return;
		}

		if(st < fixc_work_line){  //对采用fixed和绝对定位的分界点，非IE6
            if(fixc_top){
            	fixc_target.removeClass(fixc_change);
                fixc_bottom = true;
                fixc_top = false;
            }
        } else {
            if(fixc_bottom){
            	fixc_target.addClass(fixc_change);
                fixc_top = true;
                fixc_bottom = false;
            }
        }   
...
/*
 *#13 修复滑动门在IE里面的不支持active的效果
$.fn.fixed_door({
	target_ele: '.a'
});
 解释说明：
$.fn.second_menu({
	target_ele: '.a'   //滑动门是由: div > a > span组成的。这个元素是最外层的元素
});
 *
 */
 $.fn.fixed_door = function(param){
 	var fd_target = $(param.target_ele);
 	fd_target.each(function(index){     //允许多个按钮公用一个类，遍历绑定就好了
 		$(this).mousedown(function(){
 			$(this).children('a').addClass('active');    //这个类是要在CSS里面写好的，且固定为active
 		}).mouseup(function(){
...
$.fn.substr({
 ...
 解释说明：
还需要引入：http://source1.qq.com/wsd/substring.js
$.fn.substr({
	target_ele: '.a',      //目标元素
	startp: 0,             //开始截取的位置
	endp: 260              //最后截取的位置，中文算2个，英文算1个
});
 *
 */
 $.fn.substr = function(param){
 	var sub_target;
 	sub_target = $(param.target_ele);
	sub_target.each(function(index){  //给页面中每一个这个类的都采取截取的方法
		var str;
		str = $(this).html().toString();
		str = str.subCHStr(param.startp, param.endp);   
		$(this).html(str);
	});
};
...
$.fn.dropbox({
	target_ele: '.arr-down',
	dropbox_action: 'click',
	dropbox_class: 'change'
});
 解释说明：
$.fn.dropbox({
    target_ele: '.arr-down',   //外层的class
    dropbox_action: 'click',   //触发的时间，可以是（hover, click），默认是hover
    dropbox_class: 'change'    //这是在触发事件后在外层增加删除的类名
});
 *
 */
 $.fn.dropbox = function(param){
 	var dropbox_target, dropbox_action, dropbox_class;
 	dropbox_target = $(param.target_ele);
 	dropbox_action = param.dropbox_action || 'hover';
 	dropbox_class = param.dropbox_class;   

 	if(dropbox_action == 'hover'){
 		dropbox_target.hover(function(){
	        dropbox($(this).children('ul'), $(this));
	    });
 	} else {
 		dropbox_target.click(function(){
	        dropbox($(this).children('ul'), $(this));
	    });
 	}
    function dropbox(ul, wrapper){
    	if(ul.is(':visible')){  //每次点击判断元素是否已经被隐藏了，同时可以适合2种方法而且toggle有个bug，点击其他地方会触发
            ul.slideUp(function(){  //这里要设置成回调函数，不然class变化快会很奇怪
            	wrapper.removeClass(dropbox_class);
            });
            ...
            wrapper.addClass(dropbox_class);
        }
    }
};
